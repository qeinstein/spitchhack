...
async def start_call():
    ...
    try:
        ...
        gather = Gather(
            ...
            record="true"  # Enable recording to ensure RecordingUrl
        )
...
async def process_response(
    ...
    try:
        ...
        logger.info(f"Received process_response: language={language}, SpeechResult={SpeechResult}, RecordingUrl={RecordingUrl}, audio={audio}")

        if not SpeechResult and not RecordingUrl and not audio:
            logger.warning("No audio input provided, redirecting to start_call")
            twiml = VoiceResponse()
            twiml.say("I didn't hear anything. Please try again.", voice="Polly.Joanna")
            twiml.redirect("/start_call", method="POST")
            return Response(content=str(twiml), media_type="application/xml")
        ...
        if cleaned_language is not None:
            ...
            if not transcribed_text:
                logger.error("Transcription failed or returned empty text")
                twiml = VoiceResponse()
                twiml.say("I didn't understand your language choice. Please try again.", voice="Polly.Joanna")
                twiml.redirect("/start_call", method="POST")
                return Response(content=str(twiml), media_type="application/xml")
