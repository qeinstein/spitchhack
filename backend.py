...
SYSTEM_PROMPT = (
    "You are a helpful assistant named Proxy. Respond like a human. "
...
try:
    ...
    logger.error("Failed to init clients", error=str(e))
    raise
...
async def gemini_chat_reply(messages: list) -> str:
    ...
    return response.text or "Sorry, I couldn’t process that."

# ---- Root ----
@app.get("/")
async def root():
    return {"message": "Welcome to Proxy Relay API"}
...
async def voice_entry(request: Request):
    ...
    if not twilio_validator.validate(str(request.url), dict(form), sig):
...
async def process_language_fallback(request: Request):
    twiml = VoiceResponse()
    twiml.say("No input received. Redirecting to menu.")
...
async def process_language(request: Request, Digits: str = Form(None), CallSid: str = Form(None)):
    ...
    lang_name, twilio_code, spitch_code = LANGUAGE_MAP[Digits]
    LANGUAGE_SELECTION[CallSid] = (lang_name, twilio_code, spitch_code)
    ...
    relay.language(code=twilio_code, transcription_provider="google")  # Twilio transcription only
    return Response(content=str(twiml), media_type="application/xml")
...
async def health():
    try:
        ...
        spitch_status = f"down: {e}"
    try:
        _ = await gemini_chat_reply([{"role": "system", "content": "test"}])
        gemini_status = "ok"
    except Exception as e:
        gemini_status = f"down: {e}"
    return {"status": "ok", "spitch": spitch_status, "gemini": gemini_status}
...
async def relay_ws(ws: WebSocket):
    ...
    VOICES = {"yo": "kayode", "ig": "emeka", "ha": "danjuma", "en": "jude"}
    ...
    try:
        while True:
            ...
            if ev == "connected":
                call_sid, stream_sid = data["callSid"], data["streamSid"]
                history = [{"role": "system", "content": SYSTEM_PROMPT}]
                continue

            if ev == "media":
                user_text = data.get("media", {}).get("payload", "").strip()
                if not user_text:
                    continue

                _, _, lang = LANGUAGE_SELECTION.get(call_sid, ("English", "en-US", "en"))
                if lang != "en":
                    user_text = spitch_translate(user_text, source=lang, target="en")

                history.append({"role": "user", "content": user_text})
                reply_en = await gemini_chat_reply(history)
                reply_local = spitch_translate(reply_en, source="en", target=lang) if lang != "en" else reply_en

                voice_id = VOICES.get(lang, VOICES["en"])
                audio = spitch_client.speech.generate(text=reply_local, language=lang, voice=voice_id).read()
